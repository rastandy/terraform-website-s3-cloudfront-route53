...
data "template_file" "bucket_policy" {
  ...
  vars {
    bucket = "site.${replace("${replace("${var.domain.0}",".","-")}","*","star")}"
...
resource "aws_s3_bucket" "website_bucket" {
  ...
  bucket   = "site.${replace("${replace("${var.domains.0}",".","-")}","*","star")}"
  ...
  tags = "${merge("${var.tags}",map("Name", "${var.project}-${var.environment}-${replace("${var.domains.0}","*","star")}", "Environment", "${var.environment}", "Project", "${var.project}"))}"
}
...
resource "aws_iam_policy" "site_deployer_policy" {
  provider    = "aws.${var.region}"
  name        = "site.${replace("${replace("${var.domains.0}",".","-")}","*","star")}.deployer"
  path        = "/"
...
resource "aws_iam_policy_attachment" "staging-site-deployer-attach-user-policy" {
  provider   = "aws.${var.region}"
  name       = "site.${replace("${replace("${var.domains.0}",".","-")}","*","star")}-deployer-policy-attachment"
  users      = ["${var.deployer}"]
